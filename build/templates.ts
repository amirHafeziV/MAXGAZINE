...
<meta property="og:image" content="https://maxgazine.com/og-image.png">
...
<article class="wrap section" style="max-width:760px">
  ...
  <h1>${esc(article.headline[lang])}</h1>
  <p class="dek" style="font-size:20px;color:#444;line-height:1.5;margin:14px 0 8px">${esc(article.dek[lang])}</p>
...
export function renderStoriesIndex(
  ...
  const cards = articles
    .map(
      (a) => `    <a class="card" href="${prefix}${lang}/${a.slug}.html">
      <div class="cat mono">${esc(a.category.toUpperCase())} · ${esc(a.date)}</div>
      <h3>${esc(a.headline[lang])}</h3>
      <p>${esc(a.dek[lang])}</p>
